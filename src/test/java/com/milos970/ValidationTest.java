...
public class ValidationTest
{
    ...
    private int checkBalance(AVLTree.AvlNode<Integer, String> node) {
        if (node == null) {
            return -1;
        }

        int left = checkBalance((AVLTree.AvlNode<Integer, String>) node.leftSon());
        int right = checkBalance((AVLTree.AvlNode<Integer, String>) node.rightSon());
    ...
    private void testIntervalSearch(Tree<Integer, String> tree) {
        ...
        for (int i = 0; i < 100_000; ++i) {
            int min = random.nextInt(100_000);
            int max = random.nextInt(min,100001);
            List<String> values = tree.intervalSearch(min, max);
